    ...
    $(document).on('change', '#ingredient_id', function() {
        ...
        if (ingredient_details != '') {
            ...
            let cart_row = '<tr class="rowCount" data-item_id="' + ingredient_details_array[0] + '" data-id="' + suffix + '" id="row_' + suffix + '">' +
                ...
                '" name="quantity_amount[]"  data-item_id="'+ingredient_details_array[0]+'"  onfocus="this.select();" class="form-control cal_culate integerchk aligning countID"  placeholder="Qty/Amount"><span class="label_aligning">' +
                ingredient_details_array[3] + '</span><td><a class="btn btn-danger btn-xs row_delete" style="margin-left: 5px; margin-top: 10px;" ><i style="color:white" class="fa fa-trash"></i> </a></td>' +
                '</tr>';
            tab_index++;
...
$(document).on('keyup', '.cal_culate', function() {
    let from_storage_id = $("#from_storage_id").val();
    let ing_id = $(this).attr('data-item_id');
    let transfer_qty = Number($(this).val());
    ...
    let cancel = $("#cancel").val();
    let alert2 = $("#alert").val();
    let your_current_stock_is = $("#your_current_stock_is").val();

    let this_action = $(this);
    $.ajax({
        url: base_url + "Inventory/getCurrentInventory",
        method: "post",
        dataType: "json",
        async:false,
        data: {
        ...
        success: function (data) {
            let return_total_stock = Number(data.total_stock);
            let return_stock_unit = (data.stock_unit);
            if(transfer_qty>return_total_stock){
                swal({
                    title: alert2+"!",
                    text: your_current_stock_is+" "+return_total_stock+""+return_stock_unit,
                    confirmButtonText: ok,
                    confirmButtonColor: '#3c8dbc'
                });
                this_action.val('');
