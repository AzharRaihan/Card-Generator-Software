(function ($) {
  ...
  let base_url = $("base").attr("data-base");
    ...
    let self_order_table_id = $("#self_order_table_id").val();
  ...
  let gst_state_code = $("base[data-gst-state-code]").attr(
    "data-gst-state-code"
  );
  ...
  let delivery_for_customer_must_address = $(
    ...
    let pleaseselectordertypebeforeaddtocart = $("#pleaseselectordertypebeforeaddtocart").val();
    ...
    function get_all_information_of_recent_sales_from_indexeddb_print_invoice(sale_id) {
        return new Promise(function (resolve, reject) {
            ...
            objectStore.openCursor().onsuccess = function(event) {
                ...
                if (cursor) {
                    // if(cursor.value.sales_id == sale_id) {
                    if(Number(cursor.value.sale_id) == Number(sale_id)) {
                        let orderData = cursor.value;
                        resolve(orderData.order);
                    }
                    cursor.continue();
        ...
        objectStore.openCursor(null, 'prev').onsuccess = function(event) {
            ...
            if (cursor) {
                ...
                if(outlet_id_indexdb===outlet_id){
                    ...
                    let table_name = "";
                    let waiter_name = rowData.waiter_name != null ? rowData.waiter_name : "";
                    ...
                    order_list_left += '<p class="oder_list_class">Order: <span class="running_order_order_number">' + rowData.sale_no + "</span></p>";
                    ...
                    order_list_left += '<p  class="txt_4">Time Count: <span id="order_minute_count_' + sales_id + '">' + minute + '</span>:<span id="order_second_count_' + sales_id + '">' + second + "</span> M</p>";
                    order_list_left += "</div>";
                    order_list_left += "</div>";
                    order_list_left += "</div>";
    ...
    function setOrderTables(table_id_div){
        ...
        objectStore.openCursor(null, 'prev').onsuccess = function(event) {
            ...
            if (cursor) {
                ...
                let table_details = cursor.value;
                let person = Number(table_details.persons);
                let table_id = Number(table_details.table_id);
                let order_number = table_details.sale_no;
...
// purpose of this function is to delete data from future sales table by using data from indexeddb
  ...
  function getCurrentDate() {
    ...
    function print_kot_print(order_info, sale_id) {
    ...
    function call_print_invoice(order_info, sale_id) {
                            ...
                            <div id="receipt-data">
                                <div class="text-center">
                                    ...
                                    <p class="p_txt">
                                        ...
                                        `+inv_tax_registration_no+`: ` +outlet_tax_registration_no+`<br>
        ...
        for (let key in order.items) {
            ...
            let i = 1;
            total_item_counter+=Number(this_item.qty);
            let discount_value = Number(this_item.item_discount_amount) ? "(-"+inv_currency+Number(this_item.item_discount_amount).toFixed(ir_precision)+")": '';
            invoice_print+=`<tr>`;
            invoice_print+=`<td class="no-border border-bottom ir_wid_90"># `+sl+`: `+this_item.menu_name;
            invoice_print+=`<small></small> &nbsp;&nbsp;`+ this_item.qty + `&nbsp;X&nbsp;`+inv_currency+Number(this_item.menu_unit_price).toFixed(ir_precision)+discount_value ;
            if (this_item.menu_combo_items != "" && this_item.menu_combo_items!=undefined  && this_item.menu_combo_items!=null && this_item.menu_combo_items!="undefined") {
            ...
            invoice_print+=  inv_currency+Number(this_item.menu_price_with_discount).toFixed(ir_precision);
            invoice_print+=`</td>`;
            invoice_print+=`</tr>`;
            for (let mod_key in modifier_names_custom) {
                let tmp_mod_name_m_n = modifier_names_custom[mod_key];
                let tmp_mod_name_m_p = inv_currency+Number(modifier_prices_custom[mod_key]).toFixed(ir_precision);
                invoice_print+=`<tr>`;
        ...
        $.each(order.sale_vat_objects, function (key, value) {
            if(Number(value.tax_field_amount)){
                total_vat_section_to_show +=`<tr>
                                                                        ...
                                                                        <th class="text-right">
                                                                            `+inv_currency+Number(value.tax_field_amount).toFixed(ir_precision) +`
                                                                        </th>
                                                                    </tr>`;
            }
        });
        ...
        invoice_print+=`</tbody>
                                    <tfoot>
                                        ...
                                        <tr>
                                        <th  class="text_left">`+inv_currency+inv_sub_total+`
                                        
                                        </th>
                                            <th class="text-right">
                                                `+inv_currency+Number(order.sub_total).toFixed(ir_precision)+`
                                            </th>
                                        </tr>`;
        if(Number(order.total_discount_amount)){
            invoice_print+=`<tr>
                                        ...
                                        <th class="text-right">
                                            `+inv_currency+Number(order.total_discount_amount).toFixed(ir_precision)+`
                                        </th>
                                        </tr>`;
        }
        if(Number(order.delivery_charge_actual_charge)){
            invoice_print+=`<tr>
                                                ...
                                                <th class="text-right">
                                                    `+inv_currency+Number(order.delivery_charge_actual_charge).toFixed(ir_precision)+`
                                                </th>
                                                </tr>`;
        }
        if(Number(order.tips_amount_actual_charge)){
            invoice_print+=`<tr>
                                                ...
                                                <th class="text-right">
                                                    `+inv_currency+Number(order.tips_amount_actual_charge).toFixed(ir_precision)+`
                                                </th>
                                                </tr>`;
        }
        ...
        invoice_print+= `<tr>
                                        <th  class="text_left">`+inv_grand_total+`
                                            ...
                                            <th class="text-right">
                                                `+inv_currency+Number(order.total_payable).toFixed(ir_precision)+`
                                            </th>
                                        </tr>
                                        ...
                                        <th  class="text_left">`+inv_paid_amount+`
                                            ...
                                            <th class="text-right">
                                                `+inv_currency+Number(order.paid_amount).toFixed(ir_precision)+`
                                            </th>
                                        </tr>`;
        if(Number(order.due_amount)) {
            invoice_print += `<tr>
                                        <th  class="text_left">`+inv_due_amount+`
                                            ...
                                            <th class="text-right">
                                                ` + inv_currency+Number(order.due_amount ).toFixed(ir_precision)+ `
                                            </th>
                                        </tr>`;
        }
        ...
        if(Number(order.hidden_given_amount)) {
            invoice_print += `<tr>
                                        <th  class="text_left">`+inv_given_amount+`
                                            ...
                                            <th class="text-right">
                                                ` + inv_currency+Number(order.hidden_given_amount ).toFixed(ir_precision)+ `
                                            </th>
                                        </tr>`;
        }
        if(Number(order.hidden_change_amount)) {
            invoice_print += `<tr>
                                        <th  class="text_left">`+inv_change_amount+`
                                            ...
                                            <th class="text-right">
                                                ` + inv_currency+Number(order.hidden_change_amount ).toFixed(ir_precision)+ `
                                            </th>
                                        </tr>`;
        }
            ...
            $.each(obj_payment, function (key, value) {
                ...
                invoice_print += `<tr>
                                                                                <th class="text_left">`+value.payment_name+`
                                                                                    ...
                                                                                    <th class="text-right">
                                                                                        ` + inv_currency+Number(value.amount).toFixed(ir_precision) + `
                                                                                    </th>
                                                                                </tr>`;
            ...
            if(status_continue==true){
                if(is_variation=="Yes") {
                    ...
                    draw_table_for_order +=
                        row_number > 0
                            ? ""
                            : '<div data-cp_type="1" class="single_order customer_panel fix" data-id="'+item_id+'" id="order_for_item_' + item_id + '">';
                    ...
                    draw_table_for_order +=
                        ...
                        item_id +
                        '"></i> <span class="qty_item_custom 1_cp_qty_'+item_id+'" id="item_quantity_table_' +
                        item_id +
                        '">' +
                        item_quantity +
      ...
      $(document).on("click", "#add_to_cart", function (e) {
          ...
          if(is_variation_product && !status){
              ...
              draw_table_for_order +=
                  '<div class="single_order_column third_column fix"><i class="fal fa-minus decrease_item_table txt_5" id="decrease_item_table_' +
                  item_id +
                  '"></i> <span class="1_cp_qty_'+item_id+' qty_item_custom" id="item_quantity_table_' +
                  item_id +
                  '">' +
                  item_quantity +
                  '</span> <i class="fal fa-plus increase_item_table txt_5" id="increase_item_table_' +
              ...
              let product_comb = $("#item_combo_table_"+item_id).html();
    ...
    $(document).on("change", "#discount_type", function () {
      ...
      $(document).on("change", ".select_walk_in_customer_custom", function (e) {
        ...
        if(place_edit_order!="Update Order"){
            let default_discount = $(this).find(':selected').attr('data-default_discount');
            let default_discount_tmp = $(this).find(':selected').attr('data-default_discount');
        ...
        if(is_login_checker==true){
            ...
            if (selected_order_type_object.length > 0) {
                ...
                if(is_self_order=="Yes" || is_online_order=="Yes"){
                    ...
                    add_sale_by_ajax(order_info, sale_id,outlet_id_indexdb,company_id_indexdb,sale_no_new,"");
              ...
              success: function (response) {
                ...
                for (let key in response) {
                  if (i == response.length) {
                    option_customers +=
                      '<option  data-default_discount="'+response[key].default_discount+'" data-current_due="0"  data-customer_address="'+response[key].address+'"   data-customer_gst_number="'+response[key].gst_number+'"  value="' +
                      response[key].id +
                      '" selected>' +
                      response[key].name +
                      " " +
  ...
  $(document).on("click", "#print_kot", function (e) {
      ...
      if(pos_10){
          if (
              ...
              let print_type_kot = $(".print_type_kot").val();
              if (print_type_kot == "web_browser") {
                  let res_kot = get_all_information_from_indexeddb(sale_id).then(function(data){
                      // var response = jQuery.parseJSON(data);
                      if(data !== null) {
                          print_kot_print(data,sale_id);
                      } else {
                          console.log(sale_id + " => This id not found into the Database, Please reload your page");
                      }
                  });
  ...
  function add_sale_by_ajax(order_object, sale_id,outlet_id='',company_id='',sale_no_new='',is_direct_sale='') {
  ...
  function print_invoice(sale_id) {
    ...
    let print_type_invoice = $(".print_type_invoice").val();
      if (print_type_invoice == "web_browser") {
          // let res = get_all_information_from_indexeddb(sale_id).then(function(order_info){
          let res = get_all_information_of_recent_sales_from_indexeddb_print_invoice(sale_id).then(function(order_info){
