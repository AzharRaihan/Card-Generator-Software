...
$(function() {
    ...
    $(document).on('click', '.row_edit', function(e) {
        ...
        let variation_name_code = $(this).parent().parent().find("td:eq(2)").text();
        let sale_price = $(this).parent().parent().find("td:eq(3)").text();
        let sale_price_take_away = $(this).parent().parent().find("td:eq(4)").text();
        let sale_price_delivery = $(this).parent().parent().find("td:eq(5)").text();
        let vr01_loyalty_point = $(this).parent().parent().find("td:eq(6)").text();
        $("#m_sale_price").val(sale_price);
        $("#m_sale_price_take_away").val(sale_price_take_away);
        $("#m_sale_price_delivery").val(sale_price_delivery);
    ...
    $(document).on('click', '.add_variation_html', function(e) {
        ...
        if(status!=true){
            ...
            let html_prview = '<tr class="row_variation_view" id="row_variation_view'+row_id+'"></div>\n' +
                ...
                '                                            <td>\n' +
                '                                                <input type="hidden" class="variation_row_update" value="" name="variation_row_update[]"><input type="hidden" class="variation_ingrs" value="'+hidden_ingr_html+'" name="variation_ingrs[]"><input type="hidden" class="hidden_delivery_html" value="'+hidden_delivery_html+'" name="hidden_delivery_html[]"><input type="hidden" value="'+variation_name+'" name="variation_name[]"><input type="hidden" value="'+var01_grand_total_cost+'" name="var01_grand_total_cost_arr[]"><input type="hidden" value="'+vr01_loyalty_point+'" name="vr01_loyalty_point_arr[]"><input type="hidden"  value="'+m_sale_price+'" name="m_sale_price[]"><input type="hidden" value="'+m_sale_price_take_away+'" name="m_sale_price_take_away[]"><input type="hidden" value="'+m_sale_price_delivery+'" name="m_sale_price_delivery[]">'+hidden_tax_html+'\n' +
    ...
    $(document).on('change', '#food_menu_id', function() {
        ...
        if(food_menu_id){
            ...
            let cart_row = '<tr class="food_row" data-id="'+food_menu_id+'">' +
                ...
                '<td class="ir_w_23">'+food_menu+' <input type="hidden" id="food_menu_id_hidden_' + suffix +'" name="food_menu_id_hidden[]" class="food_menu_id_hidden" value="' + food_menu_id+ '||'+food_menu+'"/></td>' +
                '<td style="width: 30%"><input type="text" tabindex="' + tab_index +
