...
abstract class Model implements Arrayable, ArrayAccess, Jsonable, JsonSerializable, QueueableEntity, UrlRoutable
{
    ...
    {
        //change encryptDecrypt to $value for ignore the plan id
        return $this->where($field ?? $this->getRouteKeyName(), encryptDecrypt($value,'decrypt'))->first();
    }
    ...
    /**
     * Set scope for business dependency
     */
    public function scopeUserBusiness($query){
        $conditions = [
            'user_id' => auth()->user()->id,
            'business_id' => session()->get('business_id')
        ];
        return $query->where('user_id' , auth()->user()->id)->where('business_id' , session()->get('business_id'));
    }

    /**
     * Set scope for business dependency
     */
    public function scopeTechnicalExpert($query){
